 ...
 * 税金計算ロジック（フロント/バック共有）
 */
import { BASIC_DEDUCTION, RECONSTRUCTION_TAX_RATE, RESIDENT_TAX_RATE, RESIDENT_TAX_FLAT, PENSION_RATE, PENSION_MAX_MONTHLY_SALARY, HEALTH_INSURANCE_RATE, HEALTH_MAX_MONTHLY_SALARY, EMPLOYMENT_INSURANCE_RATE, SALARY_DEDUCTION_RULES, INCOME_TAX_BRACKETS, } from './constants.ts';
/**
 * 給与所得控除額を計算
 *
 * @param salary 給与収入（円）
 * @returns 給与所得控除額（円）
 */
export function calculateSalaryDeduction(salary) {
 ...
 * @returns 課税所得（円）
 */
export function calculateTaxableIncome(grossIncome) {
 ...
 * @returns 復興特別所得税（円）
 */
export function calculateReconstructionTax(incomeTax) {
 ...
 * @returns 住民税（円）
 */
export function calculateResidentTax(taxableIncome) {
    if (taxableIncome <= 0) {
 ...
 * @returns 社会保険料合計（円）
 */
export function calculateSocialInsurance(annualIncome) {
